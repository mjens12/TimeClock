...
/**
 * The Week class handles all actions that have to do with
 * the time worked throughout the week, Also handles the
...
public class Week {
	 ...
	 * 
	 * @param time Double value of the total
	 * hours worked.
	 */
	...
	/**
	 * Getter method for the totHrsWorked
	 * variable.
	 ...
	 * @param day Day of the week where data is changed.
	 * @param unit Field location (in Hour, Out Min, AMPM, ect.)
	 * @param dataValue int value that represents a unit
	 * of time or a value for AM/PM.
	 */
	 ...
	 * @param day Day of the week being targeted.
	 * @param unit Data location being targeted
	 ...
	 * @param day Integer value where Sun = 0, Sat = 6
	 * @param time Double value of the time worked.
	 */
	...
	/**
	 * Method that resets the weekData array.
	 * Sets each In/Out time for every week day
	 * to 12:00AM.
	 */
	...
	public double calcTime(final int time1H, final int time1M,
			final int time1AMPM, final int time2H,
			final int time2M, final int time2AMPM) {
      ...
      //Error checking for invalid inputs.
        if (time1Hour < 1 || time1Hour > 12 || time1M < 0 
        		...
        		|| time2Hour > 12 || time2M < 0 || time2M > 59) {
            return 0.00; 
        } else { //continue
